"""Vendored Kaggle pool agent: kiyotah "A sample rule-based agent (Dragapult ex deck)".

Pulled near-verbatim from the public kernel (outputs/kernels/kiyotah__a-sample-rule-
based-agent-dragapult-ex-deck) as a torch-free LEAGUE opponent — the Phantom-Dive
spread/combo archetype that best stresses a one-tank Metal control deck. Loaded by
dist_worker as the `kaggle:dragapult` opponent spec. **It carries ITS OWN deck**
(`dragapult_deck.csv`) read RELATIVE TO THIS FILE — never from the cwd `deck.csv` —
so multiple borrowed agents (each a different archetype deck) can coexist in one
worker. Exposes the standard `agent(obs)->list[int]` contract and `my_deck`; the
original entry point is kept as `_agent_impl` behind a crash-safe `agent()` wrapper
that clamps the selection to the engine's min/max bounds. Not our code — community-
shared (skarin's "Phantom Dive or Go Home" build was also pulled for reference).
"""
...
# Load THIS agent's own deck relative to this file (workers chdir to agent/, so a
# bare "deck.csv" would wrongly load the trainee's deck). Fall back to the Kaggle
# runtime path only if the sibling file is missing.
def _load_deck():
    here = os.path.dirname(os.path.abspath(__file__))
    for fp in (os.path.join(here, "dragapult_deck.csv"), "deck.csv",
               "/kaggle_simulations/agent/deck.csv"):
        if os.path.exists(fp):
            with open(fp) as f:
                return [int(x) for x in f.read().split() if x.strip()][:60]
    raise FileNotFoundError("dragapult_deck.csv")
...
# ── Crash-safe wrapper (ours) ───────────────────────────────────────────────
def agent(obs_dict):
    """Delegate to the kernel policy, but never crash and always return a legal
    selection (clamped to minCount/maxCount, deduped, in-bounds)."""
    try:
        obs = to_observation_class(obs_dict)
    except Exception:
    ...
    for i in raw or []:
        if isinstance(i, int) and 0 <= i < n and i not in seen:
            seen.add(i)
            out.append(i)
    lo = max(0, min(select.minCount, n))
    hi = max(lo, min(select.maxCount, n))
    if len(out) < lo:  # top up with any remaining legal options
        for i in range(n):
            if i not in seen:
                out.append(i)
                if len(out) >= lo:
