"""Vendored Kaggle pool agent: romanrozen 'STRONG START Baseline Agent V10' (LB 950+).

Pulled verbatim from the public kernel (outputs/kernels/romanrozen__strong-start-
baseline-agent-v10-lb-950) as a LEAGUE opponent — a strong, torch-free, Crustle-
Lucario (our deck family) rule-based agent with light 1-ply engine search. Loaded
by dist_worker as the `kaggle:romanrozen_v10` opponent spec. Exposes the standard
`agent(obs)->list[int]` contract and `my_deck`; it plays OUR deck in self-league
games (same archetype, so its heuristics transfer). Not our code — community-shared.
"""
import os
import sys
import random
