"""Vendored Kaggle pool agent: ryotasueyoshi "Rule-based (not Psychic) Alakazam"
(rule-based-not-psychic-alakazam-best-5th).

Pulled near-verbatim from the public kernel (outputs/kernels/ryotasueyoshi__rule-
based-not-psychic-alakazam-best-5th) as a torch-free LEAGUE opponent — a fast
hand-size-scaling Alakazam (Powerful Hand: 20 dmg per card in hand) with a
Kadabra/Alakazam + Dudunsparce + Fezandipiti ex draw engine. Loaded by dist_worker
as the `kaggle:alakazam` opponent spec. Added to close the ladder blind spot
(went 0-3 vs Alakazam on the first RL submission, see
rl_research/RL_LADDER_ANALYSIS_2026-06-30.md). **It carries ITS OWN deck**
(`alakazam_deck.csv`, the kernel's inline build) read RELATIVE TO THIS FILE — never
from the cwd `deck.csv` — so multiple borrowed agents can coexist in one worker.
Exposes the standard `agent(obs)->list[int]` contract and `my_deck`. Not our code —
community-shared.
"""
import os
import sys
...
def read_deck_csv():
    ...
    # so a bare "deck.csv" would wrongly load the trainee's deck). The sibling is
    # the only trusted source; the Kaggle runtime path is a pure fallback. Reject
    # anything that isn't exactly 60 cards so a truncated/cwd file can't silently
    # field the wrong deck.
    here = os.path.dirname(os.path.abspath(__file__))
    for fp in (
        os.path.join(here, "alakazam_deck.csv"),
        ...
        if os.path.exists(fp):
            ...
            if len(ids) == 60:
                return ids
    raise FileNotFoundError("alakazam_deck.csv (a legal 60-card sibling deck)")
